...
-- Блок на странице (основная единица контента)
blocks (
...
REST:
  ...
  GET    /api/blocks?page=...
  POST   /api/blocks              ← Hermes добавляет блок
  PATCH  /api/blocks/:id
  DELETE /api/blocks/:id
  PATCH  /api/blocks/reorder      ← { page_id, block_ids: [...] }
...
│ 📚 Обучение│                             │ > Что у    │
│ 💼 Работа  │  ┌─────────────────────┐    │ меня       │
│ 📰 Новости │  │ 📝 План питания     │    │ сегодня?   │
│            │  │ завтрак: овсянка    │    │            │
│            │  │ обед: курица+рис    │    │ > Добавь   │
│            │  └─────────────────────┘    │ задачу     │
│            │  ┌─────────────────────┐    │ купить     │
│            │  │ 🏃 Тренировка      │    │ продукты   │
│            │  │ Сегодня: ходьба    │    │            │
...
**Главная (Dashboard):**
- Client-side rendered, данные загружаются через `GET /api/today` при монтировании
- Виджет TodaySummary: задачи на сегодня + события + прогресс по обучению
- Виджет RecentFeed: последние 10 фид-элементов из всех топиков
- Виджет QuickActions: кнопки «Добавить задачу», «Открыть здоровье», «Открыть обучение»

**Вкладки (Topics):** слева, как в Notion. Создаются и удаляются пользователем. Hermes может предложить новый топик.
**Контент (центр):** страницы с блоками внутри выбранного топика. Блоки можно перетаскивать (drag-and-drop с optimistic update через `PATCH /api/blocks/reorder`).
**Чат Hermes (справа):** контекстно привязан к текущему топику. При старте WS-сессии Hermes получает структуру workspace: список топиков, страниц и блоков через `GET /api/topics` + `GET /api/pages`. WebSocket для стриминга ответа.
